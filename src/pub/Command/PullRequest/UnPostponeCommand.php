...
class UnPostPoneCommand extends Command {
  ...
  protected function configure() {
    $this->setName('pr-unpostpone')
      ->setDescription('Unpostponed a specific pull-request.')
      ->addArgument(
        'pull-request',
   ...
   *
   * This removes a ci:postponed label to the PR (aka issue) on github.
   *
  ...
  protected function execute(InputInterface $input, OutputInterface $output) {
    ...
    $project_config->load();
    $labels = $github->api('issue')->labels()->remove($project_config->settings['organization'] , $project_config->settings['repository'], $pr_number, self::GITHUB_LABEL);
    $pr_url = "https://github.com/{$project_config->settings['organization']}/{$project_config->settings['repository']}/pull/{$pr_number}";
    $output->writeln("<info>Pull Request: $pr_url has been un-postponed.</info>");
  }
}
